# AgentForge GitHub 内部预览发布设计

## 目标

将 AgentForge `0.1.0` 准备为 GitHub 私有仓库中的内部预览版本。GitHub Actions 必须为 macOS ARM64、macOS x64 和 Windows x64 生成可供人工安装验证的 Draft Release 产物；本阶段不提供公开分发承诺、正式代码签名、notarization 或自动更新。
...
- GitHub 仓库：`https://github.com/Aimny2020/agent-forge.git`
- 默认分支：`main`
- 仓库可见性：private
- 产品名：`AgentForge`
- 首发版本：`0.1.0`
...
`stitch_agentforge_project_manager (1)/` 下三个已缺失的旧设计导出文件正式删除。删除记录保留在工作树供用户审阅，本次不 commit。
...
工作流使用 `tauri-apps/tauri-action` 创建 `AgentForge v__VERSION__` Draft Release，上传 DMG 或 NSIS 产物，关闭 updater JSON 上传，并赋予 job `contents: write`。不配置长期 Secret；GitHub 自动提供的 `GITHUB_TOKEN` 足够创建 Draft Release。
