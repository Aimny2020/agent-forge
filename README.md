...
**The right capabilities for every project.**

AgentPalette 是一个跨工具、本地优先的桌面管理器，用于集中管理 Agents、Skills 和 Harnesses，并为每个项目配置恰到好处的 AI 能力组合。

AgentPalette is a local-first desktop workspace for managing agents, skills, and harnesses across projects and AI tools.

项目当前处于 `0.3.0` 私有预览阶段，仅供授权的内部测试人员使用，未授权公开分发。
...
- 项目共享设置未来放入 `.agentpalette/manifest.toml`；敏感值只存储引用名称，真实值进入系统 Keychain。

### 更名兼容标识

为保证从旧版本升级时继续读取已有数据，以下非展示标识暂时保持不变：Tauri bundle identifier `com.lemon.agentforge`、SQLite 文件名 `agentforge.db`、全局数据目录 `~/.agent-forge/`，以及主题存储键 `agentforge-theme`。历史预留的项目元数据目录 `.agentforge/` 仍会被识别并跳过扫描，未来的新项目共享配置使用 `.agentpalette/manifest.toml`。这些值只作为持久化兼容契约，不代表当前产品品牌；没有迁移策略前不得直接修改。
